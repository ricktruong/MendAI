...
class OpenAIAnalysisService:
    ...
    def _call_openai_vision_api(
        ...
        """
        try:
            # Determine image format from base64 data or default to JPEG
            # JPEG is more efficient for medical images and reduces token usage
            # The frontend now sends JPEG format, but we handle both for compatibility
            image_format = "jpeg"  # Default to JPEG for better compression
            
            # Check if image_data already includes format info (for backward compatibility)
            # Otherwise assume JPEG (which is what we're now sending from frontend)
...
Be specific about slice locations for all findings."""
    ...
    def _parse_slice_analysis_response(
        ...
        try:
            ...
            return self._create_error_slice_analysis(
